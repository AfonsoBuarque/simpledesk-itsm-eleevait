...
import { useCreateProblema } from '@/hooks/useProblemas';
...
const NewProblemaDialog: React.FC<NewProblemaDialogProps> = ({
  open,
  onOpenChange,
}) => {
  ...
  const createProblema = useCreateProblema();
  ...
  const onSubmit = async (data: ProblemaFormData) => {
    try {
      await createProblema.mutateAsync(data as any);
      form.reset();
      onOpenChange(false);
    } catch (error) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Registre um novo problema de infraestrutura ou serviço
          </DialogDescription>
        </DialogHeader>
...
export default NewProblemaDialog;