...
interface UseEditAtivoFormProps {
  ativo: Ativo;
  open: boolean;
  onSuccess: () => void;
}

export const useEditAtivoForm = ({ ativo, open, onSuccess }: UseEditAtivoFormProps) => {
  ...
  useEffect(() => {
    if (ativo && open) {
      ...
      const formData: AtivoFormData = {
        ...
        proprietario_id: ativo.proprietario_id || '',
  ...
  return {
    form,
    onSubmit,
    isSubmitting: updateAtivoMutation.isPending,
  };
};
