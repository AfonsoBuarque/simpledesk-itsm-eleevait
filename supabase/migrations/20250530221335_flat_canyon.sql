/*
  # Correção de políticas de acesso e funções de usuário

  1. Remoção de políticas conflitantes
  2. Adição de políticas de leitura pública
  3. Melhoria da função handle_new_user
  4. Adição de função is_admin
  5. Configuração de RLS para tabela users
*/
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'users' AND policyname = 'Usuários podem atualizar seus próprios dados'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'users' AND policyname = 'Usuários podem inserir seus próprios dados'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'time_entries' AND policyname = 'Allow public read access to time_entries'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'expenses' AND policyname = 'Allow public read access to expenses'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'tasks' AND policyname = 'Allow public read access to tasks'
  ) THEN
